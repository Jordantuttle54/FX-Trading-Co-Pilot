...
(function () {
  ...
  function injectChartPanel() {
    ...
    card.innerHTML = `
      ...
      <div class="chart-workspace">
        ...
        <aside class="chart-account-panel" id="chartAccountPanel">
          <div class="muted small">Loading paper account...</div>
        </aside>
      </div>
    `;
  ...
  function initChart() {
    ...
    window.addEventListener('resize', () => {
      if (chart && chartContainer) chart.resize(chartContainer.clientWidth || 900, chartContainer.clientHeight || height);
    });
  }
  ...
  function accountStartBalance() {
    return num(qs('quickTradeBalance')?.value, num(qs('scanBalance')?.value, 10000)) || 10000;
  }
  ...
  function realisedMoney(trade) {
    ...
    if (r !== null) return r * tradeRiskMoney(trade);
    return 0;
  }

  function estimateOpenTradeMoney(trade, pricesByPair) {
    const pair = trade.pair || activeChartMeta.pair;
    const current = num(pricesByPair[pair], null);
    const entry = num(trade.entry_price, num(trade.entry, null));
    const stop = num(trade.stop_loss, null);
    ...
    const resultR = riskDistance ? move / riskDistance : 0;
    return resultR * tradeRiskMoney(trade);
  }

  async function priceForPair(pair) {
    if (latestPricesByPair[pair]) return latestPricesByPair[pair];
    try {
      const tick = await chartApi(`/api/agent/chart/tick?pair=${encodeURIComponent(pair)}`);
      const price = num(tick.price, null);
      if (price !== null) latestPricesByPair[pair] = price;
      return price;
    } catch (_) {
      return null;
    }
  ...
  async function loadPaperAccountPanel() {
    const panel = qs('chartAccountPanel');
    if (!panel) return;
    try {
      const [openData, allTradesRaw] = await Promise.all([
        chartApi('/api/agent/trades/open'),
        chartApi('/api/agent/trades'),
      ]);
      const openTrades = openData.open_trades || openData.trades || openData.items || [];
      const allTrades = Array.isArray(allTradesRaw) ? allTradesRaw : (allTradesRaw.trades || allTradesRaw.items || []);
      const pairs = [...new Set(openTrades.map(t => t.pair).filter(Boolean))];
      await Promise.all(pairs.map(priceForPair));

      const startBalance = accountStartBalance();
      const closedTrades = allTrades.filter(t => String(t.status || '').toLowerCase() === 'closed');
      const realised = closedTrades.reduce((sum, t) => sum + realisedMoney(t), 0);
      const openPnl = openTrades.reduce((sum, t) => sum + estimateOpenTradeMoney(t, latestPricesByPair), 0);
      const balance = startBalance + realised;
      const equity = balance + openPnl;
      const totalProfit = realised + openPnl;

      const rows = openTrades.length ? openTrades.map(t => {
        const pnl = estimateOpenTradeMoney(t, latestPricesByPair);
        const current = latestPricesByPair[t.pair];
        return `
          <div class="chart-position-row">
            <div>
              <div class="chart-position-name">${escapeHtml(tradeLabel(t))}</div>
              <div class="chart-position-meta">${escapeHtml(t.pair || '')} ${escapeHtml(String(t.direction || '').toUpperCase())} | Current ${formatPrice(current)}</div>
            </div>
      ...
      panel.innerHTML = `
        <div class="chart-account-top">
          <div>
            <div class="chart-account-title">Paper Account</div>
            <div class="chart-account-sub">Estimated position money, paper mode only</div>
          </div>
          <button class="btn-secondary" style="font-size:11px;padding:6px 10px" onclick="refreshAgentAccountPanel()">Refresh</button>
        </div>
        ...
        <div class="chart-position-list">
          ${rows}
        </div>
        <div class="chart-account-note">Open P&L is estimated from the latest available quote and each trade's risk/SL distance. Closed trade profit uses saved journal results.</div>
      `;
    } catch (e) {
  ...
  async function pollLiveTick() {
    ...
    try {
      ...
      if (Date.now() - accountPanelLastRefresh > 15000) {
        accountPanelLastRefresh = Date.now();
        loadPaperAccountPanel();
      }
    } catch (e) {
      const warningBox = qs('chartWarnings');
  ...
  window.refreshAgentAccountPanel = function refreshAgentAccountPanel() {
    accountPanelLastRefresh = Date.now();
    loadPaperAccountPanel();
  };
