...
from .agent_db import (
    ...
    get_audit_log, get_strategy_versions, record_loss,
)
...
async def agent_execute(req: AgentExecuteRequest, background_tasks: BackgroundTasks, user: str = Depends(current_user)):
    ...
    if candidate["status"] != "trade_candidate":
                log_audit(
                                event_type="execution", decision="rejected",
                                reason=candidate.get("rejection_reason") or "No valid setup found.",
                                pair=req.pair, details=candidate,
                )
        raise HTTPException(status_code=422, detail={
                        "message": "Setup does not meet all rule requirements.",
    ...
    log_audit(
                event_type="execution", decision="executed",
                reason=f"Demo trade placed. Mode: {execution_result.get('mode')}.",
                pair=req.pair, trade_id=trade_id,
...
async def agent_open_trades(user: str = Depends(current_user)):
        return {"open_trades": get_open_agent_trades(), "trading_allowed": trading_allowed(), "kill_switch": kill_switch_active()}
...
async def kill_switch_on(req: KillSwitchRequest, user: str = Depends(current_user)):
    ...
    log_audit(event_type="kill_switch", decision="activated", reason=req.reason, details={"by": user})
    return result
...
async def kill_switch_off(user: str = Depends(current_user)):
    ...
    log_audit(event_type="kill_switch", decision="deactivated", reason=f"Deactivated by {user}")
    return result
