...
APP_VERSION = "0.7.0-live-paper-mvp"
...
app = FastAPI(title="AI FX Live Paper Trading MVP", version=APP_VERSION)
...
def add_audit(user: str, event_type: str, decision: str, reason: str = "", pair: str = "", trade_id: str | None = None):
    AUDIT.insert(0, {"id": str(uuid.uuid4()), "created_at": now(), "user_name": user, "event_type": event_type, "pair": pair, "trade_id": trade_id, "decision": decision, "reason": reason})
...
def get_trade(user: str, trade_id: str) -> Dict[str, Any]:
    for t in TRADES:
        if str(t["id"]) == str(trade_id) and t.get("user_name") == user:
            return t
...
@app.get("/api/health")
async def health():
    return {"status": "ok", "version": APP_VERSION, "paper_trading": True, "live_trading_enabled": False, "live_trading_locked": True, "data_provider": "oanda" if oanda_configured() else "synthetic-fallback"}

@app.get("/api/config")
async def config():
    return {"watchlist": WATCHLIST, "selected_provider": "oanda" if oanda_configured() else "synthetic-fallback", "account_currency": "GBP", "paper_trading": {"starting_balance": START_BALANCE, "enforce_london_window": ENFORCE_WINDOW, "live_trading_locked": True, "storage_mode": "memory"}, "rules": {"max_risk_per_trade_pct": MAX_RISK, "max_daily_loss_pct": DAILY_LIMIT, "max_weekly_loss_pct": WEEKLY_LIMIT, "min_risk_reward": MIN_RR, "live_trading_locked": True, "min_confidence_score": MIN_CONF}, "configured": {"oanda": oanda_configured(), "auth_passcode": bool(os.getenv("AUTH_PASSCODE")), "auth_token_secret": bool(os.getenv("AUTH_TOKEN_SECRET"))}}
...
async def agent_execute(req: AgentExecuteRequest, user: str = Depends(current_user)):
    ...
    return {"trade_id": t["id"], "execution": {"mode": "paper", "status": "filled", "order_id": t["order_id"], "live_money": False}, "candidate": c, "trade": t}
...
async def agent_manage(req: ManageTradesRequest, user: str = Depends(current_user)):
    ...
    return {"actions_taken": actions, "open_trades": list_trades(user, "open"), "snapshot": snapshot()}

@app.get("/api/agent/trades/open")
async def agent_open_trades(user: str = Depends(current_user)):
    return {"open_trades": list_trades(user, "open"), "trading_allowed": {"allowed": not KILL_SWITCH["active"], "daily_loss_pct": 0.0, "weekly_loss_pct": 0.0, "daily_limit": DAILY_LIMIT, "weekly_limit": WEEKLY_LIMIT}, "kill_switch": KILL_SWITCH["active"]}
...
async def agent_status(user: str = Depends(current_user)):
    ...
    return {"version": APP_VERSION, "mode": "paper", "paper_trading": True, "live_trading_locked": True, "kill_switch_active": KILL_SWITCH["active"], "trading_allowed": {"allowed": not KILL_SWITCH["active"], "daily_loss_pct": 0.0, "weekly_loss_pct": 0.0, "daily_limit": DAILY_LIMIT, "weekly_limit": WEEKLY_LIMIT, "reason": KILL_SWITCH["reason"]}, "open_trade_count": len(open_trades), "open_trades": open_trades, "managed_actions": actions, "london_window_now": london_window(), "data_provider": "oanda" if oanda_configured() else "synthetic-fallback", "timestamp": now()}
