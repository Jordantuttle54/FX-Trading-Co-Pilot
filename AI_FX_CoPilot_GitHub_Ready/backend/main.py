...
from .database import init_db, add_journal, list_journal, clear_journal, add_paper_trade, list_paper_trades, close_paper_trade
...
APP_VERSION = "0.4.0-github-ready"
...
async def health():
    return {
        ...
        "message": "Live order execution is locked. Paper/manual review only.",
        "confidence_gate": {
...
async def api_add_journal(entry: JournalEntryIn):
    return add_journal(entry.model_dump())

@app.delete("/api/journal")
async def api_clear_journal():
    clear_journal()
    return {"status": "cleared"}

@app.get("/api/paper-trades")
async def api_list_paper_trades():
    return {"rows": list_paper_trades()}

@app.post("/api/paper-trades")
async def api_add_paper_trade(trade: PaperTradeIn):
    return add_paper_trade(trade.model_dump())

@app.post("/api/paper-trades/{trade_id}/close")
async def api_close_paper_trade(trade_id: int, close_price: float, result_r: float):
    return close_paper_trade(trade_id, close_price, result_r)
